...
// Initialize OpenCC converter for Simplified to Traditional Chinese (Hong Kong)
...
function main() {
  console.log(
    'Starting Simplified Chinese to Traditional Chinese (HK) translation...'
  );

  if (!fs.existsSync(sourceDir)) {
    console.error(`Source directory not found: ${sourceDir}`);
    // If source doesn't exist, we can't translate.
    // However, in the user's workflow, they might want to run this AFTER creating zh-Hans.
    // We'll exit gracefully if it's missing but warn loudly.
    process.exit(1);
  }
  ...
  console.log(`Simplified Chinese (zh-Hans) → Traditional Chinese (zh-Hant)`);
}
